...
def test_quiniela_mismo_numero_repetido_en_resultado_paga_cada_posicion(app_mod):
    """Resultado 38-23-23, jugada 23, monto 100 → q2 + q3 = RD$1,200."""
    app = app_mod
    monto = 100.0
    q2 = float(app.PAGOS["quiniela_2"])
    q3 = float(app.PAGOS["quiniela_3"])
    esperado = monto * q2 + monto * q3
    assert abs(esperado - 1200.0) < 0.01

    p = float(app.calcular_premio("Quiniela", "23", monto, "38", "23", "23") or 0)
    assert abs(p - esperado) < 0.02

    Pm = app._merge_pagos_calc(app.PAGOS)
    frags = app._quiniela_premio_fragmentos("23", monto, "38", "23", "23", Pm)
    shards = sorted(f["premio_shard"] for f in frags)
    assert shards == ["q2:23", "q3:23"]
    assert abs(sum(float(f["premio"]) for f in frags) - esperado) < 0.02
    ...
    c = app.db()
    cur = c.cursor()
    _seed_resultado_row(app, cur, lot_ld, drw_ld, fe_ld, "38", "23", "23")
    tid, lid = _seed_ticket_line(
        app, cur, "23", monto, fecha_sorteo=fe_ld, lottery=lot_ld, draw=drw_ld
    )
    c.commit()
    ...
    filas = [x for x in lista if int(x.get("line_id") or 0) == int(lid)]
    assert len(filas) == 2
    shards_lista = sorted(x.get("premio_shard") for x in filas)
    assert shards_lista == ["q2:23", "q3:23"]
    total = sum(float(x.get("premio") or 0) for x in filas)
    assert abs(total - esperado) < 0.05

    pres_q2 = app._ganadores_acierto_presentacion(
        "Quiniela", "23", "38", "23", "23", monto * q2, pagos=app.PAGOS, premio_shard="q2:23"
    )
    pres_q3 = app._ganadores_acierto_presentacion(
        "Quiniela", "23", "38", "23", "23", monto * q3, pagos=app.PAGOS, premio_shard="q3:23"
    )
    assert pres_q2["acierto_ok"] and "2do premio" in (pres_q2.get("detalle") or "")
    assert pres_q3["acierto_ok"] and "3er premio" in (pres_q3.get("detalle") or "")

    techo = app._ganadores_premio_techo_legal(
        "Quiniela", monto, pagos=app.PAGOS, numero="23", r1="38", r2="23", r3="23"
    )
    assert abs(float(techo) - esperado) < 0.02
    assert app._ganadores_premio_integridad(
        "Quiniela", "23", monto, "38", "23", "23", esperado, pagos=app.PAGOS
    )


def test_quiniela_mismo_numero_en_primera_y_segunda(app_mod):
    """Resultado 23-23-38, jugada 23, monto 100 → 1ra (80) + 2da (8) = RD$8,800."""
    app = app_mod
    monto = 100.0
    q1 = float(app.PAGOS["quiniela_1"])
    q2 = float(app.PAGOS["quiniela_2"])
    esperado = monto * q1 + monto * q2
    assert abs(esperado - 8800.0) < 0.01

    p = float(app.calcular_premio("Quiniela", "23", monto, "23", "23", "38") or 0)
    assert abs(p - esperado) < 0.02

    Pm = app._merge_pagos_calc(app.PAGOS)
    frags = app._quiniela_premio_fragmentos("23", monto, "23", "23", "38", Pm)
    assert sorted(f["premio_shard"] for f in frags) == ["q1:23", "q2:23"]


def test_quiniela_mismo_numero_en_las_tres_posiciones(app_mod):
    """Resultado 23-23-23, jugada 23, monto 100 → 1ra + 2da + 3ra = RD$9,200."""
    app = app_mod
    monto = 100.0
    q1 = float(app.PAGOS["quiniela_1"])
    q2 = float(app.PAGOS["quiniela_2"])
    q3 = float(app.PAGOS["quiniela_3"])
    esperado = monto * q1 + monto * q2 + monto * q3
    assert abs(esperado - 9200.0) < 0.01

    p = float(app.calcular_premio("Quiniela", "23", monto, "23", "23", "23") or 0)
    assert abs(p - esperado) < 0.02

    Pm = app._merge_pagos_calc(app.PAGOS)
    frags = app._quiniela_premio_fragmentos("23", monto, "23", "23", "23", Pm)
    assert sorted(f["premio_shard"] for f in frags) == ["q1:23", "q2:23", "q3:23"]

    techo = app._ganadores_premio_techo_legal(
        "Quiniela", monto, pagos=app.PAGOS, numero="23", r1="23", r2="23", r3="23"
    )
    assert abs(float(techo) - esperado) < 0.02
