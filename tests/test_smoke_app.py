...
def test_api_ventas_cajeros_filtra_usuarios_historicos_y_activos(app_mod, client):
    c = app_mod.db()
    cur = c.cursor()
    cur.execute(
        app_mod._sql(
            """
            INSERT INTO users (id, username, role, approved, is_approved, is_blocked, activo)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
            """
        ),
        (1201, "cajero_activo_rank", "cajero", 1, 1, 0, 1),
    )
    cur.execute(
        app_mod._sql(
            """
            INSERT INTO tickets (cajero, monto, created_at)
            VALUES (%s, %s, %s)
            """
        ),
        ("cajero_activo_rank", 250, app_mod.timestamp_rd_iso()),
    )
    cur.execute(
        app_mod._sql(
            """
        ...
        ("paloma_borrada", 999, app_mod.timestamp_rd_iso()),
    )
    c.commit()
    c.close()
    ...
    assert r.status_code == 200
    data = r.get_json()
    nombres = {str(x.get("cajero") or "") for x in (data.get("cajeros") or [])}
    assert "cajero_activo_rank" in nombres
    assert "paloma_borrada" not in nombres


def test_api_ventas_cajeros_excluye_cajero_inactivo(app_mod, client):
    c = app_mod.db()
    cur = c.cursor()
    cur.execute(
        app_mod._sql(
        ...
        (1202, "anabel_inactiva", "cajero", 1, 1, 0, 0),
    )
    cur.execute(
        app_mod._sql(
            """
        ...
        ("anabel_inactiva", 700, app_mod.timestamp_rd_iso()),
    )
    c.commit()
    c.close()
    ...
    data = r.get_json()
    nombres = {str(x.get("cajero") or "") for x in (data.get("cajeros") or [])}
    assert "anabel_inactiva" not in nombres
